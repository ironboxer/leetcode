"""
https://leetcode-cn.com/problems/predict-the-winner/s
...
class Solution:
    ...
    def play(self, nums, low, high):
        if low > high:
            return 0
        A = nums[low] - self.play(nums, low + 1, high)
        B = nums[high] - self.play(nums, low, high - 1)
        return max(A, B)
